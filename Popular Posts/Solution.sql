...
with cte as (
		select 
			u.session_id, 
			v.post_id post_id, 
			v.perc_viewed perc_viewed, 
			DATEDIFF(second, session_starttime, session_endtime) session_time  from user_sessions u
		join post_views v
		on u.session_id = v.session_id
		)
select 
	post_id, 
	sum((perc_viewed * session_time) /100) total_viewtime 
from cte
group by post_id
having sum((perc_viewed * session_time) /100) > 5
