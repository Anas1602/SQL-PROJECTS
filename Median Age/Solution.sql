...
with cte as (
			select 
				*,
				...
				cast (count(id) over(partition by country order by age rows between unbounded preceding and unbounded following) as decimal) as total_ppl
			from people
			)
select country, age from cte 
where age_rnk <= total_ppl / 2 + 1 and age_rnk >= total_ppl /2
